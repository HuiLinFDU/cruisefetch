...
from cruiser.math import multi_one_hot

class HierarchicalSequenceLoss(tf.keras.losses.Loss):
    '''
    Seq2Seq sequence loss to train Cruiser to make every timestep produce
...
class HierarchicalCrossEntropyWithLogitsLoss(tf.keras.losses.Loss):
    '''
    Hierarchical CrossEntropy loss that Cruiser was trained with in original paper
    '''
    def __init__(self, multi_label, num_offsets=64):
